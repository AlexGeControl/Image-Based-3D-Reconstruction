...
void pose_p3p_kneip (
    ...
    {   
        ...
        std::cout << real_roots[i] << ", " << cot_alpha << std::endl;
...
void PoseP3PKneip(
    ...
    for (size_t i = 0; i < result.size(); ++i) {
        ...
        //
        math::Matrix3d R;
        R[0] = T_camera(0, 0); R[1] = T_camera(0, 1); R[2] = T_camera(0, 2);
        R[3] = T_camera(1, 0); R[4] = T_camera(1, 1); R[5] = T_camera(1, 2);
        R[6] = T_camera(2, 0); R[7] = T_camera(2, 1); R[8] = T_camera(2, 2); 
        math::Vec3d C;
        C[0] = T_camera(0, 3); C[1] = T_camera(1, 3); C[2] = T_camera(2, 3);

        solutions.push_back( R.hstack(C) );
    }
}
 ...
 */
void compute_p3p (
    sfm::Correspondences2D3D const& corresp,
